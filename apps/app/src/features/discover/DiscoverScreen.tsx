  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.searchRow}>
        <View style={[styles.searchBox, { borderColor: colors.border }]}><Ionicons name="search" size={20} color={colors.textMuted} /><TextInput value={query} onChangeText={setQuery} onSubmitEditing={search} returnKeyType="search" autoFocus placeholder={`${ENTRY_LABEL[kind]} 제목을 검색하세요`} placeholderTextColor={colors.textMuted} accessibilityLabel={`${ENTRY_LABEL[kind]} 검색`} style={[styles.searchInput, { color: colors.text }]} />{query ? <AnimatedPressable accessibilityRole="button" accessibilityLabel="검색어 지우기" hitSlop={8} onPress={() => setQuery('')} style={styles.clearSearch} pressedOpacity={0.64} scaleTo={0.9}><Ionicons name="close-circle" size={19} color={colors.textMuted} /></AnimatedPressable> : null}</View>
        <AnimatedPressable accessibilityRole="button" accessibilityLabel="검색 실행" accessibilityState={{ busy: loading, disabled: !query.trim() || loading }} disabled={!query.trim() || loading} onPress={() => { Haptics.selectionAsync().catch(() => undefined); search(); }} style={[styles.searchButton, { opacity: !query.trim() || loading ? 0.4 : 1 }]} pressedOpacity={0.7} scaleTo={0.96}>{loading ? <ActivityIndicator size="small" color={colors.primary} /> : <Text style={[styles.searchButtonText, { color: colors.primary }]}>찾기</Text>}</AnimatedPressable>
      </View>
      {kind === 'movie' && !process.env.EXPO_PUBLIC_TMDB_API_KEY ? <Text style={[styles.help, { color: colors.textMuted }]}>영화 검색 준비 중이에요. 지금은 직접 등록해 주세요.</Text> : null}
      {loading ? <SearchResultsSkeleton /> : (
        ...
        )} ListEmptyComponent={<View style={styles.empty}><Text style={[styles.emptyEyebrow, { color: colors.primary }]}>{error ? '연결 확인' : '기억 찾기'}</Text><Text style={[styles.emptyTitle, { color: colors.text }]}>{error ? '검색을 잠시 쉬어갈게요' : searched ? '검색 결과가 없어요' : `${ENTRY_LABEL[kind]} 제목을 입력해보세요`}</Text><Text style={[styles.emptyBody, { color: colors.textMuted }]}>{error || (searched ? '직접 입력해 기록을 이어갈 수 있어요.' : '제목으로 찾거나, 아래에서 바로 기록을 시작하세요.')}</Text>{error ? <AnimatedPressable accessibilityRole="button" onPress={search} style={[styles.retry, { borderColor: colors.primary }]} pressedOpacity={0.72} scaleTo={0.98}><Text style={[styles.retryText, { color: colors.primary }]}>다시 검색</Text></AnimatedPressable> : null}</View>} />
      )}
      <AnimatedPressable accessibilityRole="button" accessibilityLabel={`${ENTRY_LABEL[kind]} 직접 등록`} onPress={() => { Haptics.selectionAsync().catch(() => undefined); openForm(); }} style={[styles.manual, { backgroundColor: colors.primary, bottom: Math.max(insets.bottom, 10) }]} pressedOpacity={0.84} scaleTo={0.985}><Text style={styles.manualText}>직접 기록하기</Text></AnimatedPressable>
    </View>
  );
}
...
const styles = StyleSheet.create({ root: { flex: 1 }, searchRow: { flexDirection: 'row', paddingHorizontal: 18, paddingTop: 8, paddingBottom: 14, gap: 8 }, searchBox: { flex: 1, height: 48, borderBottomWidth: 1.5, flexDirection: 'row', alignItems: 'center' }, searchInput: { ...typography.body, flex: 1, height: '100%', paddingLeft: 9 }, clearSearch: { width: 36, height: 44, alignItems: 'flex-end', justifyContent: 'center' }, searchButton: { height: 48, minWidth: 52, alignItems: 'flex-end', justifyContent: 'center' }, searchButtonText: typography.button, help: { ...typography.caption, paddingHorizontal: 18, marginBottom: 4 }, list: { paddingHorizontal: 18, paddingTop: 16, paddingBottom: 128 }, emptyList: { flexGrow: 1 }, resultHeader: { minHeight: 30, paddingBottom: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, resultCount: typography.label, resultHint: typography.overline, result: { borderBottomWidth: StyleSheet.hairlineWidth, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', gap: 12 }, cover: { width: 54, height: 72, borderRadius: 6 }, coverPlaceholder: { alignItems: 'center', justifyContent: 'center' }, resultText: { flex: 1, gap: 4 }, resultTitle: typography.itemTitle, empty: { flex: 1, justifyContent: 'center', alignItems: 'flex-start', paddingHorizontal: 2, paddingBottom: 100, gap: 7 }, emptyEyebrow: typography.overline, emptyTitle: typography.sectionTitle, emptyBody: { ...typography.body, maxWidth: 300 }, retry: { minHeight: 44, marginTop: 10, paddingHorizontal: 16, borderRadius: 10, borderWidth: 1, alignItems: 'center', justifyContent: 'center' }, retryText: typography.button, manual: { position: 'absolute', left: 16, right: 16, height: 52, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, manualText: { ...typography.button, color: '#FFFFFF' }, skeletonList: { padding: 18, gap: 10 }, skeletonHeader: { width: 72, height: 13, borderRadius: 4, marginBottom: 2 }, skeletonText: { flex: 1, gap: 9 }, skeletonTitle: { width: '78%', height: 18, borderRadius: 4 }, skeletonMeta: { width: '48%', height: 13, borderRadius: 4 } });
